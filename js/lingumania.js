...
(function (w, d, u) {
    ...
    w.Lingumania.onSegmentTranslated = function (handler) {
        return w.Lingumania.dispatchHandlers.push(handler);
    };

    w.Lingumania.segmentTranslated = function (e) {
        var handler, i, len, ref;
        ref = w.Lingumania.dispatchHandlers;
    ...
    var event = new CustomEvent('onSegmentTranslated', { translated: w.Lingumania.translated });
    ...
    function translateDOM() {
        if (linguJSON) {
            ...
            if (linguJSON.translated_segments && linguJSON.translated_segments.length > 0 && isTranslated && currlangcode != sourceLang.lang_code) {
                ...
                w.Lingumania.toTranslate = translatedSegments;
                w.Lingumania.treeBuilt = true;

                while (node = domWalker.nextNode()) {
                    if(node.parentElement.id == 'coming' && node.parentElement.lang != currlangcode)
                        node.parentElement.lang = currlangcode;
                                ...
                                for (var i = 0; i < translatedSegments.length; i++) {
                                    ...
                                    if (translatedSegments[i].source == node.nodeValue.trim() && translatedSegments[i].target) {
                                        ...
                                        if (target.match(/<\/?\w+((\s+\w+(\s*=\s*(?:'.*?'|'.*?'|[\^''>\s]+))?)+\s*|\s*)\/?>/gim)) {
                                            ...
                                            w.Lingumania.translated++;

                                            w.Lingumania.segmentTranslated(w.Lingumania.translated);
                                        }
                                        break;
                                    }
                                }
